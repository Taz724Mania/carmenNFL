...
-- SELECT countrycode FROM countrylanguage GROUP BY countrycode HAVING COUNT(DISTINCT language) = 1 AND MAX(language) = 'Italian'; SMR & VAT
...
-- SELECT name FROM city WHERE countrycode = 'SMR'; Serravalle & San Marino
...
-- SELECT name FROM city WHERE SOUNDEX(name) = SOUNDEX('Serravalle'); didn't work
-- SELECT name FROM city WHERE levenshtein_distance(name, 'Serravalle') <= 5; didn't work
-- SELECT name FROM city WHERE name LIKE '%Serra%' OR name LIKE '%Serr____%' OR name LIKE '%Ser _____%';
-- SELECT * FROM city WHERE name = 'Serra'; just wanted to check that Serra was in South America
...
-- SELECT capital FROM country WHERE code = 'BRA'; 211
-- SELECT name FROM city WHERE id = '211'; Brasilia
...
-- SELECT name, head_coach FROM teams WHERE conference = 'NFC' AND division = 'North' OR conference = 'AFC' and division = 'South';
